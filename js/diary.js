...
function renderDiaryBook() {
    ...
    book.id = 'diaryBook';
    book.style.cssText = 'width:100%;height:100%;position:relative;transform-style:preserve-3d;transition:transform 0.6s cubic-bezier(0.4,0.2,0.2,1);transform:' + (diaryIsLocked ? 'rotateY(-2deg) rotateX(1deg)' : 'rotateY(8deg) rotateX(1deg)') + ';box-shadow:0 25px 50px rgba(0,0,0,0.3),0 0 0 1px rgba(255,255,255,0.1) inset;border-radius:16px 4px 4px 16px;';
    ...
    hdr.style.cssText = 'border-bottom:1px solid #d0c8b8;padding-bottom:0.4rem;margin-bottom:0.8rem;display:flex;justify-content:space-between;font-size:0.8rem;font-weight:400;letter-spacing:0.5px;color:#7a6e5e;text-transform:uppercase;';
    hdr.innerHTML = '<span id="pageDate">📅 —</span><span id="pageTime">✎ <span contenteditable="false" id="timeInput">entry</span></span>';
    ...
    ftr.style.cssText = 'margin-top:0.8rem;display:flex;justify-content:space-between;align-items:center;font-size:0.75rem;color:#8f826e;border-top:1px solid #ddd6c8;padding-top:0.5rem;font-weight:400;letter-spacing:0.3px;';
    ftr.innerHTML = '<span class="edit-hint" id="editHint" style="font-size:0.65rem;color:#a0907a;opacity:0.6;">🔒 sample page</span>' +
        ...
        '<button id="nextPage" style="background:transparent;border:1px solid #c6b8a4;color:#3d342b;font-size:0.75rem;padding:4px 16px;border-radius:30px;cursor:pointer;">+ new page ▶</button></div>';
...
function renderDiaryPage(index) {
    ...
    if (!diaryIsLocked && p.editable !== false) {
        ...
        if (eh) eh.textContent = p.editable===false ? '🔒 sample page' : '🔒 locked';
    }
}
...
function setupDiaryEvents(book, pm) {
    function toggleLock() {
        if (diaryIsLocked) {
            if (!diaryPin) { pm.style.display = 'flex'; return; }
            diaryIsLocked = false;
            isOpening = true;
    ...
    document.getElementById('pinSubmit').addEventListener('click', function() {
        ...
        if (code.length !== 4) { if (pe) pe.textContent = '❌ enter exactly 4 digits'; return; }
        if (!diaryPin) {
            diaryPin = code; localStorage.setItem('winchu_diary_pin', diaryPin);
            pm.style.display = 'none'; diaryIsLocked = false;
            isOpening = true; book.classList.add('opening');
            setTimeout(function() { book.classList.remove('opening','closed'); book.classList.add('open'); book.style.transform = 'rotateY(8deg) rotateX(1deg)'; isOpening = false; updateDiaryUI(book); }, 900);
            pins.forEach(function(inp) { inp.value = ''; });
            ...
            if (pe) pe.textContent = '❌ incorrect PIN';
            pins.forEach(function(inp) { inp.value = ''; }); pins[0].focus();
        }
    });
...
function saveDiaryEntry() {
    ...
    S.diary.unshift(de);
    firebase.database().ref('diary/' + S.username).push(de);
    saveState();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (dp) { var ob = new MutationObserver(function(ms) { ms.forEach(function(m) { if (m.target.classList.contains('active')) setTimeout(initDiary, 200); }); }); ob.observe(dp, { attributes: true, attributeFilter: ['class'] }); }
});
