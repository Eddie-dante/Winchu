// Diary Module - Blue Book with PIN, page navigation, Firebase sync - COMPLETE WORKING VERSION
...
function loadDiaryEntries() {
    ...
    console.log('Diary pages loaded:', diaryPages.length);
}
...
function renderDiaryBook() {
    ...
    if (!container) {
        console.log('Diary container not found');
        return;
    }
    
    console.log('Rendering diary book...');
    container.innerHTML = '';
    
    // Create all elements programmatically
    var wrapper = document.createElement('div');
    wrapper.style.cssText = 'width:100%;max-width:820px;aspect-ratio:3/2;perspective:2000px;cursor:default;margin:0 auto;position:relative;';
    ...
    footer.innerHTML = '<span id="editHint" style="font-size:0.65rem;color:#a0907a;font-weight:400;opacity:0.6;">🔒 locked</span>' +
        ...
        '<span id="pageIndicator" style="font-size:0.75rem;color:#6a5e4e;min-width:50px;text-align:center;">1 / ' + diaryPages.length + '</span>' +
    ...
    lockArea.innerHTML = '<div id="lockIcon" style="font-size:1.6rem;background:#4a7a9a;padding:0 14px;border-radius:20px;line-height:1.2;box-shadow:inset 0 -2px 0 #1a3a5c;cursor:pointer;transition:0.15s;color:#e8f0f8;">' + (diaryIsLocked ? '🔒' : '🔓') + '</div>' +
        '<button id="lockToggle" style="background:#4a7a9a;border:none;color:#e8f0f8;font-size:0.7rem;padding:5px 18px;border-radius:20px;cursor:pointer;letter-spacing:1px;text-transform:uppercase;box-shadow:0 2px 0 #1a3a5c;">' + (diaryIsLocked ? 'unlock' : 'lock') + '</button>' +
    ...
    pinModal.innerHTML = '<div style="background:#f0ece6;padding:2rem 2.2rem;border-radius:24px;box-shadow:0 30px 50px rgba(0,0,0,0.5);text-align:center;max-width:340px;width:100%;border:1px solid #c6d0dc;">' +
        ...
        '<input type="password" maxlength="1" class="pin-digit" style="width:48px;height:56px;text-align:center;font-size:1.6rem;border:1px solid #b0bcc8;border-radius:12px;background:#fff;color:#1a3a5c;outline:none;" />' +
        '</div>' +
        '<button id="pinSubmit" style="background:#4a7a9a;border:none;color:#e8f0f8;font-size:0.85rem;padding:10px 20px;border-radius:40px;cursor:pointer;letter-spacing:1px;box-shadow:0 3px 0 #1a3a5c;width:100%;">' + (!diaryPin ? 'set PIN & open' : 'unlock') + '</button>' +
    ...
    console.log('Diary book rendered');
    
    // Render first page
    renderDiaryPage(diaryCurrentPage);
    
    // Setup events
    setupDiaryEvents(book, pinModal);
}
...
// SETUP DIARY EVENTS
// ============================================================
function setupDiaryEvents(book, pinModal) {
    console.log('Setting up diary events...');
    ...
    console.log('Diary events setup complete');
}
...
function saveDiaryEntry() {
    ...
    firebase.database().ref('diary/' + S.username).push(diaryEntry);
    saveState();
        ...
        var observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.target.classList.contains('active')) {
                    console.log('Diary page activated');
