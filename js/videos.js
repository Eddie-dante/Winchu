// Videos Module - Fixed likes and comments
...
function loadVideos() {
    ...
    ref.on('child_added', function(snapshot) {
        var video = snapshot.val(); var key = snapshot.key;
    ...
    ref.on('child_changed', function(snapshot) {
        var video = snapshot.val(); if (!video) return; video.id = snapshot.key;
        if (!Array.isArray(video.likes)) video.likes = [];
        if (!Array.isArray(video.comments)) video.comments = [];
...
function setupVideoAutoplay(container) {
    ...
    var observer = new IntersectionObserver(function(entries) { entries.forEach(function(entry) { if (entry.isIntersecting) { entry.target.muted = false; entry.target.play().catch(function() { entry.target.muted = true; entry.target.play().catch(function() {}); }); } else { entry.target.pause(); } }); }, { threshold: 0.6 });
...
function handleVideoUpload(event) {
    ...
    if (!file.type.startsWith('video/')) { toast('Select a video'); event.target.value = ''; return; }
    if (file.size > 50*1024*1024) { toast('Max 50MB'); event.target.value = ''; return; }
    toast('Uploading...');
    var reader = new FileReader();
    reader.onload = function(e) {
        firebase.database().ref('videos').push({ author: S.username, avatar: S.avatar || null, text: '', url: e.target.result, time: new Date().toISOString(), likes: [], comments: [] });
        toast('Uploaded!');
    };
...
function likeVideo(videoId) {
    ...
    firebase.database().ref('videos/' + videoId + '/likes').once('value').then(function(snapshot) {
        ...
        return firebase.database().ref('videos/' + videoId + '/likes').set(likes);
    }).then(function() {
...
function commentVideo(videoId) {
    ...
    showDialog({ emoji: '💬', title: 'Comment', placeholder: 'Write...', confirmText: 'Post' }).then(function(r) {
        if (r && r.trim()) {
            firebase.database().ref('videos/' + videoId + '/comments').once('value').then(function(s) {
                var c = s.val() || [];
                if (!Array.isArray(c)) c = [];
                c.push({ username: S.username, text: r.trim(), time: new Date().toISOString() });
                return firebase.database().ref('videos/' + videoId + '/comments').set(c);
            }).then(function() { toast('Comment added!'); });
        }
    });
}
...
console.log('🎬 Videos loaded - likes and comments fixed');